...
local function run(func) task.spawn(function() local suc, err = pcall(function() func() end) if not suc then warningNotification('Vape', 'Failed to load module: '..tostring(debug.traceback(err)), 999) end end) end
...
run(function()
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			local _, subbed = pcall(function() return game:HttpGet('https://github.com/sstvskids/whitelists') end)
...
run(function()
    ...
    WatermarkText = Watermark.CreateTextBox({
        Name = "MainText",
        TempText = "Skid-War text",
        FocusLost = function(enter)
            if enter then
                if TextLabel then
                    TextLabel.Text = WatermarkText.Value == "" and "SkidWare" or tostring(WatermarkText.Value)
    ...
    CreditsText = Watermark.CreateTextBox({
        Name = "CreditsText",
        TempText = "Skid-War bottom text",
        FocusLost = function(enter)
            if enter then
                if TextLabel_2 then
                    TextLabel_2.Text = CreditsText.Value == "" and "by @._stav, @qwerty, @chang3d | version: nextgen" or tostring(CreditsText.Value)
    ...
    local GUIFonts = {"GothamBold"}
    local GUIFonts2 = {"Gotham"}
	for i,v in pairs(Enum.Font:GetEnumItems()) do
		if v.Name ~= "GothamBold" then
			table.insert(GUIFonts, v.Name)
		end
		if v.Name ~= "Gotham" then
			table.insert(GUIFonts2, v.Name)
		end
	end
    TextGUI_1 = Watermark.CreateDropdown({
        Name = "Font",
        List = GUIFonts,
        Function = function(val)
            if TextLabel then
                TextLabel.Font = Enum.Font[val]
            end
        end
    })
    TextGUI_2 = Watermark.CreateDropdown({
        Name = "CreditFont",
        List = GUIFonts2,
        Function = function(val)
            if TextLabel_2 then
                TextLabel_2.Font = Enum.Font[val]
            end
        end
    })
end)
...
--[[run(function()
	local HeadModification = {Enabled = false}
				...
				task.spawn(function()
					if Headless.Enabled then
						repeat
							task.wait()
							Head.Transparency = 1
						until (not Headless.Enabled or not HeadModification.Enabled)
					else
						Head.Transparency = 0
					end
					if HeadFlash.Enabled then
						repeat
							task.wait(HeadFlashDelay.Value)
							Head.Transparency = 1
							task.wait(HeadFlashDelay.Value)
							Head.Transparency = 0
						until (not HeadFlash.Enabled or not HeadModification.Enabled)
					else
						Head.Transparency = 0
					end
				end)
	...
	Headless = HeadModification.CreateToggle({
		Name = "Headless",
		Function = function(val)
			Headless.Enabled = val
		end
	})
	HeadFlash = HeadModification.CreateToggle({
		Name = "HeadFlash",
		Function = function(val)
			HeadFlash.Enabled = val
		end
	})
	HeadFlashDelay = HeadModification.CreateSlider({
		Name = "FlashDelay",
		Function = function(val)
			HeadFlashDelay.Value = val
		end,
		Min = 0,
		Max = 2,
		Default = 0
...
run(function()
    local LightingModifications = {Enabled = false}
	local ShadowsVal = {Enabled = false}
	local BlurOption = {Enabled = false}
    local Time = {Value = 24}
	local FogStartVal = {Value = 200}
	local FogEndVal = {Value = 200}
	local BlurVal = {Value = 10}
	local ShadowSoftnessVal = {Value = 10}
	local lightingsettingsold = {
    ...
    LightingModifications = wingui.render({
        ...
        Function = function(callback)
            if callback then
				repeat task.wait() until game:IsLoaded() and callback
                lightingService.TimeOfDay = Time.Value
				if ShadowsVal.Enabled then
					lightingService.GlobalShadows = true
				else
                    lightingService.GlobalShadows = false
                end
				lightingService.FogStart = FogStartVal.Value
				lightingService.FogEnd = FogEndVal.Value
				lightingService.ShadowSoftness = ShadowSoftnessVal.Value
				if BlurOption.Enabled then
					pcall(function()
						LightingBlur = Instance.new("BlurEffect")
						LightingBlur.Parent = lightingService
						LightingBlur.Size = BlurVal.Value
					end)
				else
                    LightingBlur:Destroy()
                end
            else
                lightingService.TimeOfDay = lightingsettingsold.TimeOfDay
				lightingService.GlobalShadows = true
				lightingService.FogStart = lightingsettingsold.FogStart
				lightingService.FogEnd = lightingsettingsold.FogEnd
				lightingService.ShadowSoftness = lightingsettingsold.ShadowSoftness
				pcall(function()
					LightingBlur:Destroy()
				end)
            end
	...
	ShadowsVal = LightingModifications.CreateToggle({
		Name = "Shadows",
		Function = function(val)
			ShadowsVal.Enabled = val
		end
	})
	BlurOption = LightingModifications.CreateToggle({
		Name = "Blur",
		Function = function(val)
			BlurOption.Enabled = val
		end
	})
    Time = LightingModifications.CreateSlider({
        Name = "Time",
        Min = 0,
        Max = 24,
        Function = function(val) end,
	...
	FogStartVal = LightingModifications.CreateSlider({
        Name = "FogStart",
        Min = 0,
        Max = 200,
        Function = function(val) end,
	...
	FogEndVal = LightingModifications.CreateSlider({
        Name = "FogEnd",
        Min = 0,
        Max = 200,
        Function = function(val) end,
	...
	BlurVal = LightingModifications.CreateSlider({
        Name = "Blur",
        Min = 0,
        Max = 10,
        Function = function(val) end,
	...
	ShadowSoftnessVal = LightingModifications.CreateSlider({
        Name = "ShadowSoftness",
        Min = 0,
        Max = 10,
        Function = function(val) end,
...
run(function()
    ...
    ChatFreezer = wingui.exploit({
        ...
        Function = function(callback)
            if callback then
                task.spawn(function()
                    local Message = ChatSayPhrases and #ChatSayPhrases.ObjectList > 0 and ChatSayPhrases.ObjectList[1] or "⛓"
                    local Unicode = " "
                    Message = Message .. Unicode:rep(FastMessages.Value - #Message)
                    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    ...
    FastMessages = ChatFreezer.CreateSlider({
        Name = "MessageSpeed",
        Min = 1,
        Max = 200,
        Function = function(val) end, 
